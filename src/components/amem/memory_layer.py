...
import time
from components.memory import MyDocument, DocFilter
from agent.baseagent import ThreadTaskRunner
...
class MemoryNote:
    ...
    def __init__(self, 
        ...
        self.cid = cid or "default"
        self.type = type or {"original"}
...
# ================
    ...
    def build_memory_by_turn(self, turn: HistoryItem):
            turn_datatime = turn.date
            conversation_tmp = "Speaker "+ turn.role + "says : " + self.history.getContentByID(turn.contentID)
            kwargs = {
                "cid": turn.contentID,
                "role": turn.role,
                "sid": turn.sessionID
            }
            self.add_note(conversation_tmp,time=turn_datatime, **kwargs)
                           
    def load(self, store_output_path: str):
        ...
        with open(store_output_path, 'rb') as f:
            cached_memories = pickle.load(f)
        # Restore memories to agent
        self.memories = cached_memories
        if os.path.exists(retriever_cache_file):
            self.retriever = self.retriever.load(retriever_cache_file,retriever_cache_embeddings_file)
        else:
            self.retriever = self.retriever.load_from_local_memory(cached_memories, 'all-MiniLM-L6-v2')
    ...
    def similarity_search_with_score(
        ...
        **kwargs: Any,
        ...
        _, ids = self.find_related_memories(query[0], k)
        res_list: List[tuple[MyDocument, float]] = []
        for id in ids:
            note: MemoryNote = self.memories[id]
            doc = MyDocument(
                id=note.cid,
                page_content=note.content,
                metadata={
                    "type": {"original"},
                    "retrieved_by": {"question"},
                    "sessionID": note.sid,
                    "date": note.timestamp,
                    "role": note.role,
                    "contentID": note.cid,
...
#==================
    ...
    def add_note(self, content: str, time: str = None, **kwargs) -> str:
        ...
        with self.thread_lock:
            ids = self.retriever.add_documents(["content:" + note.content + " context:" + note.context + " keywords: " + ", ".join(note.keywords) + " tags: " + ", ".join(note.tags)])
            self.memories[ids[0]] = note
            note.id = ids[0]
            if evo_label == True:
                self.evo_cnt += 1
    ...
    def consolidate_memories(self):
        ...
        for memory in self.memories.values():
            ...
            ids = self.retriever.add_documents([memory.content + " , " + metadata_text])
            memory.id = ids[0]
