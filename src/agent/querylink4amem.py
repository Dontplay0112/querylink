...
from components.types import HistoryItem
...
from components.amem.memory_layer import AgenticMemorySystem

import threading
import os

class QueryLinkAmemAgent(BaseAgent):
    ...
    def init(self, dataItem: DataItem):
        ...
        # NOTE DEBUG Agentic Memory System
        ...
        if self.config.agent.agent == "QueryLinkAmemLocomo":
    ...
    def _build_memory(self):
        ...
        retriever_cache_file = Path(store_output_path).with_suffix('.retriever_cache.pkl')
        retriever_cache_embeddings_file = Path(store_output_path).with_suffix('.retriever_cache_embeddings.pkl')
        ...
        past_retriever_cache_file = Path(past_store_output_path).with_suffix('.retriever_cache.pkl')
        past_retriever_cache_embeddings_file = Path(past_store_output_path).with_suffix('.retriever_cache_embeddings.pkl')
        ...
        if self.config.agent.memory_version and os.path.exists(past_store_output_path):
            # 复制过去的向量库到当前路径store_output_path的dir目录
            ...
            try:
                ...
                shutil.copy(past_retriever_cache_file, retriever_cache_file)
                shutil.copy(past_retriever_cache_embeddings_file, retriever_cache_embeddings_file)
        ...
        # 否则重新构建向量库
            ...
            ThreadTaskRunner(
                ...
                func=self.memory.build_memory_by_turn,
                msg_queue=self.msg_queue,
                res_queue=None
            ).run(max_workers=self.config.exp.max_thread_workers, max_inflight=self.config.exp.max_thread_inflight)
            
            if os.path.exists(Path(store_output_path).parent) == False:
                os.makedirs(Path(store_output_path).parent)
            with open(store_output_path, 'wb') as f:
                pickle.dump(self.memory.memories, f)
            self.memory.retriever.save(retriever_cache_file,retriever_cache_embeddings_file)
...
class InvokeGraph:
    ...
    retriever access.
    """
    def __init__(self, llm: LLMProxy, memory: AgenticMemorySystem):
        self.llm: LLMProxy = llm
        self.memory: AgenticMemorySystem = memory
    ...
    def invoke(self, qa: QA):
        ...
        # tqdm.write("\n" + str(qa.questionID))
        # tqdm.write(str(qa.evidenceCIDs))
        # tqdm.write(str(qa.question))
        # retrieved_doc_strs = []
        # for doc in res["retrieved_docs"]:
        #     content = self.memory.getContentByID(doc.metadata.get("contentID", ""))
        #     retrieved_doc_strs.append(f"- {doc.metadata.get('contentID', '')}: {content.strip().replace(chr(10), ' ')}")
        # tqdm.write("Retrieved Docs:\n" + "\n".join(retrieved_doc_strs))
        
        # envidence_doc = []
        # for ecid in qa.evidenceCIDs:
        #     content = self.memory.getContentByID(ecid)
        #     if content:
        #         envidence_doc.append(f"- {ecid}: {content.strip().replace(chr(10), ' ')}")
        # tqdm.write("Evidence Docs:\n" + "\n".join(envidence_doc))
        # tqdm.write("Answer:\n" + str(res["answer"]))
        # tqdm.write("Answer:\n" + str(qa.answer))
    ...
    # Nodes
    # def _summary_entity(self, state: InvokeState):
    #     PROMPT = f"Summarize the entities in the following question for retrieval:\n" + \
    #     f"\n\"{state['qa'].question}\"\n\n" + \
    #     f"Answer with a list of entities, separated by commas.\n"
    ...
    #     return {"retrieve_base": {"entities": entities}, "input_tokens": in_token, "output_tokens": out_token, "total_tokens": total_token}
    ...
    def _summary_keywords(self, state: InvokeState):
        ...
        f"\n\"{state['qa'].question}\"\n\n" + \
        f"Answer with a list of keywords and entities, separated by commas.\n"
        msg = self.llm.invoke([{"role": "user", "content": PROMPT}])
        res = get_msg_content(msg)
        in_token, out_token, total_token = get_msg_token(msg)
    ...
    def _guess_possible_answers(self, state: InvokeState):
        PROMPT = f"Guess the possible answers based on the following question for retrieval:\n" + \
        f"\n\"{state['qa'].question}\"\n\n" + \
        f"Answer with a list of at most three possible answers, separated by commas.\n"
        msg = self.llm.invoke([{"role": "user", "content": PROMPT}])
        res = get_msg_content(msg)
        in_token, out_token, total_token = get_msg_token(msg)
    ...
    def _retrieve(self, state: InvokeState):
        ...
        # NOTE DEBUG TEST: 只保留特定类型memory进行检索
        def filter_func(doc: MyDocument) -> bool:
            return True
            return "mix" not in doc.metadata.get("type", "") and "event" not in doc.metadata.get("type", "") and "info" not in doc.metadata.get("type", "")
        # filter_func: DocFilter = DocFilter({"type": {DocFilter.INCLUDE: "original"}})
        # filter_func: DocFilter|None = DocFilter()
        
        unique_docs_dict: Dict[str, MyDocument] = {}
        for key, value in state["retrieve_base"].items():
            for v in value:
                query_results: List[Tuple[MyDocument, float]] = self.memory.similarity_search_with_score([v],k=3, filter=filter_func)
        ...
        for ucid in unique_cids:
            before_after_k_items = self.memory.getBeforeAfterKItems(ucid, k=2)
    ...
    def _generate_answer(self, state: InvokeState):
        ...
        f"- Question: \"What did the user say on the anniversary date?\"\nDetailed Answer: Not mentioned\nBrief Answer: not mentioned\n\n" + \
